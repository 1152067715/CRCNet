import sys
sys.path.append("./libraries/CRCNet")
import os 
import argparse
import numpy as np
import torch
...
from libraries.CRCNet.utils.loss import DeepSupervisionLoss
from libraries.CRCNet.utils.metrics import evaluate,evaluate_single
...
def parse_arguments():
  parse = argparse.ArgumentParser(description='CRCNet Polyp Segmentation')
  ...
  parse.add_argument('--epoch_start', type=int, default=10)
...
if __name__ == '__main__':
    ...
    model = CRCNetModel(CONFIG).to(CONFIG["device"])
    print(model)
